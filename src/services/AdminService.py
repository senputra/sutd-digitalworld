...
class AdminService:
    ...
    def __init__(self):
        ...
        self.undo_occupy_machine = np.vectorize(
            self.__undo_occupy_machine, cache=True)
        ...
        while type(self.__root) == type(None):  # modify to use try catch instead
            try:
                ...
                self._logger.log(
                    "serviceAccountKey.json is not setup properly")
                time.sleep(0.5)
    ...
    def ID_splicer(self, full_ID):
        if type(full_ID) == list:
            ...
            for string in full_ID:
                if type(string) != str:
                    raise Exception(
                        "Invalid argument, please use a list of IDs with only strings")
                    break
                else:
                    temp = string.split('_')
                    if len(temp) != 3:
                        raise Exception(
                            "Invalid ID formot, your ID format should be: Location_Type_No.ID ")
                    else:
    ...
    def __change_Availability(self, Location: str, Type: str, no_ID: str, status: str):
        ...
        if existing_Machine != None:
            ...
            if Location != "" and Type != "" and no_ID == "":
                for machine in existing_Machine:
                    if machine.find(Location) != -1 and machine.find(Type) != -1:
                        self.__root.child(machine).child(
                            "availability").set(str(status))
            ...
            # Change availability the machine with specified full ID of Location_Type_ID
                ...
                self.__root.child(mac_ID).child(
                    "availability").set(str(status))

            else:
                # FORCE UPSERT SORRY LONG!
                self.__add_Machine(Location, Type, no_ID)
                self.__change_Availability(Location, Type, no_ID, status)
                # raise Exception(
                #     "Invalid ID. The specified machine ID does not exist")
        else:
    ...
    def __clear_latest_datalog(self, Location: str, Type: str, no_ID: str):
        ...
        if Location != "" and Type != "" and no_ID == "":
            for machine in existing_Machine:
                if machine.find(Location) != -1 and machine.find(Type) != -1:
                    latest_attempt = list(self.__root.child(machine).child(
                        "datalog").order_by_key().limit_to_last(1).get().keys())
                    self._logger.log(latest_attempt)
                    self.__root.child(machine).child(
                        "datalog").child(latest_attempt[0]).delete()
            ...
            latest_attempt = list(self.__root.child(mac_ID).child(
                "datalog").order_by_key().limit_to_last(1).get().keys())
            self._logger.log(latest_attempt)
            self.__root.child(mac_ID).child(
                "datalog").child(latest_attempt[0]).delete()
